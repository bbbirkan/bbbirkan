...
def build_lines(stats: dict[str, str]) -> tuple[dict, ...]:
    ...
    return (
        command("whoami"),
        output([("value", "Birkan Kalyon"), ("muted", " · Engineering Lead @ "),
                ("accent", "Anvilon LLC"), ("muted", " · Salem, NJ")]),
        gap(),

        command("cat motd.txt"),
        output([("muted", "I haul freight across the US by day and ship iOS and web")]),
        ...
        output([("accent", "waymind.app       "), ("muted", "Swift · SwiftUI · MapKit      "),
                ("ok", "App Store")]),
        output([("accent", "anvilon.us        "), ("muted", "Next.js 15 · TS · PocketBase  "),
                ("ok", "live")]),
        ...
        output([("value", "web     "), ("muted", "next.js · typescript · tailwind · pocketbase")]),
        output([("value", "agents  "), ("muted", "python · claude api · n8n · whisper")]),
        output([("value", "infra   "), ("muted", "docker · coolify · caddy · contabo")]),
        gap(),
